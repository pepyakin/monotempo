...
| Path | Purpose |
| --- | --- |
| `MODULE.bazel` | The module: rules_rust, the Rust and LLVM toolchains, and the single crate_universe repository `@crates` shared by every project. |
...
| `bazel/cargo/` | Generated: the Bazel Cargo workspace, one Cargo workspace (and `Cargo.lock`) over every project's crates, which crate_universe resolves. |
...
from every project at once, not written by hand. Bazel itself resolves all
projects as one Cargo workspace (`bazel/cargo/`, also generated) so that they
